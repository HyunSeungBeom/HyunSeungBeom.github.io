...
### 왜 필요한가?
- Reflow와 Repaint는 웹 페이지의 레이아웃과 시각적 속성을 업데이트할 때 발생하는 두 가지 अलग 프로세스다. 이전 방식에서는 이러한 프로세스에 대한 이해가 부족하여 성능 문제가 자주 발생했다.
...
- DOM 노드를 수정하기 전에 DOM 노드를 문서에서 분리하고, 수정 完後에 다시 문서에 붙여 넣는 방법을 사용할 수 있다.
